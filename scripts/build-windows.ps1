param([switch]$Test, [switch]$Check, [switch]$Debug)
$ErrorActionPreference = 'Stop'
$root = Split-Path $PSScriptRoot -Parent
Set-Location $root
if ($Test -and $Check) { throw 'Choose either -Test or -Check.' }
New-Item -ItemType Directory -Force (Join-Path $root '.local') | Out-Null
try {
    $buildLock = [IO.File]::Open((Join-Path $root '.local/build-windows.lock'), [IO.FileMode]::OpenOrCreate, [IO.FileAccess]::ReadWrite, [IO.FileShare]::None)
} catch {
    throw 'Another Articulate build is running in this checkout. Wait for it to finish before starting another.'
}
try {
    ...
    $cargoOptions = @('--locked', '--features', 'dynamic-backends', '--bin', 'articulate')
    if (!$Debug) { $cargoOptions += '--release' }
    if ($Test) { & cargo test @cargoOptions }
    elseif ($Check) { & cargo clippy @cargoOptions --all-targets -- -D warnings }
    else { & cargo build @cargoOptions }
    ...
    if (!$Test -and !$Check) {
        ...
        & node (Join-Path $PSScriptRoot 'collect-ui-license-notices.mjs') (Join-Path $root 'ui') (Join-Path $binaryDirectory 'licenses\javascript')
