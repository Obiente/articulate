...
- **Keep the conversation.** Open [Notetaker](docs/notetaker.md) to write personal notes, capture calls, and return to saved meetings. Read the live transcript alongside one editable Notes document, with automatic local updates when the notes model is installed.
