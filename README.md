...
Articulate is MIT licensed. Bundled fonts, icons, runtimes and Rust dependencies retain their own notices. Qwen model weights are Apache-2.0; Sortformer weights use the NVIDIA Open Model License. Weights are downloaded separately. See [visual asset notices](assets/LICENSES.md) and the `licenses/` directory inside each binary distribution.
