...
The worker divides grouped speaker turns into exact sentence excerpts and checks each against the selected model's actual tokenizer limits. It reports sentences that cannot fit rather than cutting away part of a negation or condition. The transcript itself stays grouped and unchanged. Assort then uses token-aware windows with neighboring passages, inference, and source-passage selection. Excerpts retain their parent speaker turn's full time range; no word-level timings are invented.

Returned text, speaker, source ranges, and timestamps must exactly match the input. Unknown, overlapping, duplicated, reordered, changed, or over-budget quotes are rejected. Reviewers can include or exclude individual suggestions before saving notes. Saving requires at least one selected passage and an unchanged source transcript. An empty model result is a valid abstention.
...
Finished dictation has a **Review vocabulary in context** panel. Its **Check saved spellings** button uses the included correction model. This model scores two fixed choices, `keep_original` and `replace`, for an existing enabled vocabulary rule. Its inputs include the original phrase, saved spelling, nearby recognized text, destination app, and the rule's context cues. Text inside that prompt is never parsed into authorization or replacement instructions.

The default dictionary pipeline remains deterministic. Assort's review shows whether its model favors the existing correction, the original phrase, or neither clearly. Only an explicit choice can change the finished preview. Acceptance rechecks the original recognition, current preview, app, unchanged saved rule, and a unique matching phrase; stale or ambiguous results require a fresh review or manual edit. Repeated occurrences are omitted when the review cannot identify a single intended replacement. Choosing a spelling already present is a valid no-op, and other review candidates remain available after an accepted choice, with the updated preview rechecked before the next change. This does not change text already inserted into another app, create vocabulary entries, or alter unrelated cleanup. Explicit English cues about quoting or spelling a phrase literally suppress a replacement recommendation without changing the model scores or the available manual choices. Protected numbers, currency and English negations cannot be changed by this review path. These checks do not claim full multilingual semantic understanding.
