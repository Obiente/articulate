use crate::{
    audio, call_capture, calls, classification, cleanup, discord, engine, model, speakers,
};
use anyhow::{Context, Result};
use std::{path::PathBuf, time::Instant};

pub fn run(args: &[String]) -> Result<()> {
    if args == ["--install-audio-context"] {
        crate::sensevoice::install(|_| {})?;
        println!("SenseVoice audio context installed and verified.");
        return Ok(());
    }
    if args.first().is_some_and(|arg| arg == "--assort-worker") {
        return classification::worker(&args[1..]);
    }
    if args == ["--verify-assort-models"] {
        classification::verify_builtin_models()?;
        println!("Pretrained Assort notes and correction models are bundled and verified.");
        return Ok(());
    }
    if args == ["--verify-native-audio-payload"] {
        anyhow::ensure!(
