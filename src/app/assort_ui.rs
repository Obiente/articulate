...
impl super::App {
    ...
    pub(super) fn assort_correction_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.collapsing("Review vocabulary in context", |ui| {
            ...
            if self.assort.correction_task.is_some() {
                ...
                egui::Button::new("Review vocabulary")).clicked() {
            ...
            if let Some(review) = &self.assort.correction_review {
                for score in &review.scores {
                    ...
                    ui.label(if score.replace_score >= 0.65 { "The model favors the saved spelling here." }
                        else if score.replace_score <= 0.35 { "The model favors the original wording here." }
                        else { "The model is unsure about this choice." });
                    ui.small(&candidate.context);
                    ui.horizontal_wrapped(|ui| {
                        if ui.button("Use original wording").clicked() { choice = Some((score.candidate, false)); }
            ...
            if let Some((index, saved)) = choice
                ...
                let raw = if self.raw.is_empty() { &self.assort.correction_baseline } else { &self.raw };
                match crate::correction_context::apply_choice(review, index, saved, crate::correction_context::Preview { raw, text: &self.text, baseline: &self.assort.correction_baseline }, &self.settings.entries, self.dictation_app.as_deref()) {
                    Ok(text) => {
                        ...
                        self.assort.correction_review = None; self.assort.correction_error = None;
...
pub(super) fn show(
    ...
    egui::Frame::new().fill(super::theme::SURFACE)
        ...
        .show(ui, |ui| {
            ui.label(RichText::new("Notes from your conversation").strong().size(18.0));
            ui.label("Find decisions, actions and key facts with the included local model. Review the passages before saving them.");
            ...
            if state.busy() {
                ...
                ui.ctx().request_repaint_after(std::time::Duration::from_millis(100));
            } else if ui.add_enabled(can_start && input.is_some() && state.configuration.available(ModelTask::Notes), egui::Button::new("Create notes")).clicked() && let Some(input) = input {
                    state.error = None;
                    state.summary = None;
            ...
            if let Some(summary) = &state.summary {
                if summary.highlights.is_empty() { ui.label("No passages met this model's selection threshold."); }
                for highlight in &summary.highlights {
                    ...
                    let source = &highlight.source;
                    ui.separator();
                    ui.label(RichText::new(format!("{kind} · {:02}:{:02} · {}", source.start_ms/60_000, source.start_ms/1000%60, source.speaker.as_deref().unwrap_or("Uncertain speaker"))).color(super::ACCENT));
                    ui.label(&source.text);
                }
                if !unchanged { ui.small("The transcript changed. Classify it again before saving these suggestions."); }
                if ui.add_enabled(unchanged && !summary.highlights.is_empty() && can_start, egui::Button::new("Use these highlights")).clicked() { accepted = state.summary.take(); }
            }
        });
    accepted
}
